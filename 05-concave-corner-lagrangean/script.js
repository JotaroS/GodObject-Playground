...
function draw() {
    ...
    if (isSegmentActive) {
        ...
        let flag = (!s1.isPointOutOfSegment(godobjectPoint) && !s2.isPointOutOfSegment(godobjectPoint));
        debugArr.push(s1.isPointOutOfSegment(godobjectPoint))
        debugArr.push(s1.isPointOutOfSegment(godobjectPoint))
        ...
        const b = [godobjectPoint.x, godobjectPoint.y, -c1, -c2];
        const x = math.lusolve(A,b);
        // console.log(x);
        godobjectPoint = createVector(x[0][0], x[1][0]);
        // const mouseVector = createVector(mouseX, mouseY);
        
        if(flag)godObjectPosition = godobjectPoint;
    }
